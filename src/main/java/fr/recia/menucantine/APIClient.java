...
public class APIClient {
     ...
     */
    public Mono<DynamicURLResponse> initializeAndGetDynamicEndpoint(String uai){
     ...
     */
    public Mono<AuthResponse> authenticateAndGetToken(String url) {
     ...
     */
    public Mono<ServiceDTO> makeAuthenticatedApiCallGetMenuInternal(String url, String uai, String datemenu, int service) {
    ...
    public ServiceDTO makeAuthenticatedApiCallGetMenu(String uai, String datemenu, int service) {
        ...
        if(!dynamicURL.containsKey(uai)){
            ...
            dynamicURL.put(uai, initializeAndGetDynamicEndpoint(uai).block().getContenu());
        }
        ...
        try {
            serviceDTO = makeAuthenticatedApiCallGetMenuInternal(url, uai, datemenu, service).block();
            ...
            if(webClientResponseException.getStatusCode().value() == 401){
                ...
                this.authToken = authenticateAndGetToken(url).block().getToken();
